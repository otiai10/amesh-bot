...
func (b *Bot) Help(ctx context.Context, client service.ISlackClient, event slackevents.AppMentionEvent) *CommandError {
	...
	if _, err := client.PostMessage(ctx, msg); err != nil {
		return NewCommandError(err, ":warning: ヘルプメッセージの送信に失敗しました。")
	}
	return nil
}
